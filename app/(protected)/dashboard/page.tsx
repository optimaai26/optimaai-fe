...
export default function DashboardPage() {
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Dashboard" description="Overview of your key business metrics." />
      ...
      {isLoading ? (
        ...
        <ul className="space-y-2">
          {(Array.isArray(data) ? data : []).slice(0, 5).map((row) => (
            <li key={row.id} className="flex items-center justify-between gap-2 text-sm py-1.5">
              <div className="flex items-center gap-2 min-w-0">
                <span className="inline-block w-2 h-2 rounded-full bg-amber-500 shrink-0" />
                <span className="text-foreground/85 truncate">
                  Dataset uploaded: <strong className="text-foreground">{row.file}</strong>
                </span>
              </div>
              <span className="text-xs text-muted-foreground shrink-0">
                {row.uploaded_at ? formatRelative(row.uploaded_at) : ''}
              </span>
            </li>
          ))}
        </ul>
