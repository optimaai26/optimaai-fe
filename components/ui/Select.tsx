			...
			<button
				...
				className={cn(
					...
					disabled
						...
						: "hover:border-primary/50 cursor-pointer shadow-sm hover:shadow-primary/5",
							...
							return (
								<li key={option.value}>
									<button
										...
										className={cn(
											"flex items-center justify-between w-full px-3 py-2.5 text-sm cursor-pointer transition-colors rounded-lg",
											option.disabled
												? "opacity-50 cursor-not-allowed"
												: "hover:bg-surface-100 dark:hover:bg-surface-700 text-surface-900 dark:text-surface-50",
											isSelected &&
												"bg-primary-500/10 text-primary-500 dark:bg-primary-500/20 dark:text-primary-400 font-semibold",
										)}
										onClick={() => {
											if (!option.disabled) {
