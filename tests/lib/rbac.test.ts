...
describe('useRbac logic', () => {
    ...
    it('properly guards fine-grained permissions based on roles', () => {
        ...
        expect(result.current.hasAllPermissions(['datasets:read', 'admin:users'])).toBe(false);
        expect(result.current.hasAnyPermission(['reports:export', 'admin:users'])).toBe(true);
    });
});
