...
import { CheckCircle2, Download, Inbox, Loader2, Sparkles, XCircle } from 'lucide-react';
import { useState } from 'react';
import { useOdooStatus, useOdooSync } from '@/features/odoo/useOdoo';
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Pull records from Odoo, run them through the preprocessing pipeline, and register them
            as a new dataset.
          </p>
        </div>
        <button
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <Toggle checked={includeSales} onChange={setIncludeSales} label="Sales orders" />
            <Toggle checked={includeInvoices} onChange={setIncludeInvoices} label="Invoices" />
            <Toggle checked={includeLeads} onChange={setIncludeLeads} label="CRM leads" />
          </div>
        </div>
      ...
      {sync.isError && (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-danger mb-1">Sync failed</div>
            <p className="text-sm text-muted-foreground break-words">{extractError(sync.error)}</p>
          </div>
        </div>
      )}
    </div>
...
function SyncResult({ result }: { result: NonNullable<ReturnType<typeof useOdooSync>['data']> }) {
  if (result.status === 'queued') {
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-primary mb-1">Sync queued</div>
          <p className="text-sm text-muted-foreground">
            {result.message ??
  ...
  if (result.status === 'empty') {
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-warning mb-1">No records</div>
          <p className="text-sm text-muted-foreground">
            {result.message ??
  ...
  return (
    <div className="mt-5 rounded-xl border border-success/30 bg-success/5 p-4" role="status">
      <div className="flex items-center gap-2 text-success font-semibold mb-3">
        <CheckCircle2 className="w-4 h-4" />
        Sync completed
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
        <Stat label="Rows" value={(result.rows ?? 0).toLocaleString()} />
        <Stat label="Columns" value={(result.columns ?? 0).toLocaleString()} />
        <Stat
          label="Quality before"
          value={result.quality_before != null ? `${Math.round(result.quality_before)}%` : '—'}
        />
        <Stat
          label="Quality after"
          value={result.quality_after != null ? `${Math.round(result.quality_after)}%` : '—'}
        />
      </div>
      {result.file_name && (
...
function ModeChip({
  ...
  return (
    ...
    >
      <div className={`font-medium text-sm mb-0.5 ${active ? 'text-primary' : 'text-foreground'}`}>
        {title}
      </div>
