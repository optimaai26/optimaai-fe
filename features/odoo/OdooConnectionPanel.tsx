...
import { useOdooStatus, useTestOdooConnection } from '@/features/odoo/useOdoo';
    ...
    <div className="glass-card rounded-2xl p-6">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Credentials are managed in the backend environment. Use this panel to verify
      ...
      {statusLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <ConfigField
            ...
            label="Auth method"
            value={status?.auth_method === 'api_key' ? 'API key' : 'Password'}
          />
        </div>
      )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {(
              ...
              <div key={key} className="rounded-lg bg-background/40 p-3">
                ...
                <div className="text-xl font-bold">{test.data.counts[key].toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {test.isError && (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-danger mb-1">Connection failed</div>
            <p className="text-sm text-muted-foreground break-words">{extractError(test.error)}</p>
          </div>
        </div>
      )}
      ...
      {!status?.configured && !statusLoading && (
        <div
          ...
          role="status"
        >
          Odoo isn&apos;t configured yet. Set <code>ODOO_URL</code>, <code>ODOO_DB</code>,{' '}
          <code>ODOO_USERNAME</code>, and <code>ODOO_PASSWORD</code> (or <code>ODOO_API_KEY</code>)
          in the backend <code>.env</code> file, then restart the API.
        </div>
      )}
    </div>
  );
