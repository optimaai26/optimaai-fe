...
import { ArrowUpRight, FileText, History, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="glass-card rounded-2xl p-6">
      ...
      {isLoading ? (
        ...
        <div className="text-sm text-danger">Failed to load sync history.</div>
      ) : rows.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/30 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="text-right font-semibold px-4 py-2.5">Rows</th>
                <th className="text-right font-semibold px-4 py-2.5">Cols</th>
                <th className="text-right font-semibold px-4 py-2.5">Quality</th>
              ...
              {rows.map((row) => (
                ...
                >
                  <td className="px-4 py-3 font-medium">
                    <Link href={`/datasets/${row.id}`} className="hover:text-primary">
                      {row.file}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums">{row.rows.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right tabular-nums">
                    {row.columns.toLocaleString()}
                  </td>
                  <td className="px-4 py-3 text-right text-muted-foreground tabular-nums">
                    {formatQuality(row.quality_before, row.quality_after)}
                  </td>
