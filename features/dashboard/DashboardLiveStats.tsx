...
function DataSourceBadge({ data }: { data: DashboardStats }) {
    ...
    return (
        <div className="glass-card rounded-xl border border-border px-4 py-2.5 text-xs text-muted-foreground flex items-center gap-3 flex-wrap">
            ...
            <span>
                Computed from{' '}
                <strong className="text-foreground">{data.source_file}</strong>
                {' '}· uploaded {uploadedAt}
                {' '}· currency <strong className="text-foreground">{data.currency}</strong>
            </span>
        </div>
    );
}
        ...
        >
            <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                    ...
                    <p className="mt-2 text-2xl font-bold tabular-nums truncate">
                        {value}
                    </p>
                </div>
                <div
    ...
    return (
        ...
        >
            <ResponsiveContainer width="100%" height={260}>
                ...
                >
                    <CartesianGrid stroke="currentColor" strokeDasharray="3 3" opacity={0.1} horizontal={false} />
                    <XAxis
                        type="number"
                        tick={{ fontSize: 11 }}
                        tickLine={false}
...
function ChartShell({
    ...
    return (
        <div
            className={`glass-card rounded-2xl border border-border p-5 ${className}`}
        >
    ...
    return (
        <div className="glass-card rounded-2xl border border-border p-8 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
                {message ??
                    'Upload at least one CSV through the Datasets page to populate your dashboard.'}
            </p>
            <a
                href="/datasets"
