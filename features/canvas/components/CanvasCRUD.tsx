...
import { useAddCanvasBlock, useUpdateCanvasBlock, useDeleteCanvasBlock } from '@/features/canvas/useCanvas';
...
export function CanvasCRUD({ blocks }: { blocks: CanvasBlock[] }) {
    ...
    const [edits, setEdits] = useState<Record<string, { content: string; section: CanvasSection }>>({});
    ...
    const handleAdd = () => {
        addBlock.mutate({ content: '', section: 'value_propositions', order: blocks.length + 1 }, {
            onSuccess: () => toast({ type: 'success', title: 'Block Added' })
        });
    };

    const handleBlockEdit = (id: string, newContent: string, newSection: CanvasSection) => {
    ...
    return (
        <div className="space-y-6 relative pb-20">
            <div className="flex justify-end">
                ...
                >
                    {addBlock.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                    Add Canvas Block
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {blocks.map((block) => (
                    <CanvasBlockEditor 
                        ...
                        onChange={(content, section) => handleBlockEdit(block.id, content, section)}
                ...
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 animate-slide-up">
                    <div className="glass-card shadow-2xl rounded-full p-2 flex items-center gap-4 border border-primary/20 bg-background/80 backdrop-blur-md">
                        <div className="px-4 text-sm font-medium">
                            <span className="text-primary mr-1">{Object.keys(edits).length}</span> unsaved block(s)
                        </div>
                        <button
                            onClick={() => setEdits({})}
...
function CanvasBlockEditor({ 
    ...
    return (
        <div className={cn("glass-card rounded-2xl p-5 space-y-3 transition-all duration-300", isDirty ? "border-primary/50 shadow-[0_0_15px_-3px_rgba(var(--primary),0.2)]" : "hover:border-border/80")}>
            <div className="mb-2">
                <Select
                    value={block.section}
                    onChange={(val) => onChange(block.content, val as CanvasSection)}
                    options={CANVAS_SECTIONS.map(s => ({ value: s, label: s.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) }))}
                />
            </div>
            <textarea
                value={block.content}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border/30">
                ...
                >
                    {deleteBlock.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                    Delete
                </button>
            </div>
        </div>
