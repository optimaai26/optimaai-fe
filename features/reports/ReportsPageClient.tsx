...
export function ReportsPageClient() {
    ...
    return (
        <div className="animate-fade-in space-y-6">
            <PageHeader title="Executive Reports" description="Manage and export your automated business reports." actions={actions} />
            {isLoading ? <div className="min-h-[240px] flex items-center justify-center"><Loader2 className="w-6 h-6 animate-spin text-primary" /></div> : isError ? <div className="text-sm text-danger">Failed to load reports.</div> : <DataTable columns={columns} data={reports} keyExtractor={(row) => row.id} emptyMessage="No reports generated yet" />}
        </div>
    );
}
