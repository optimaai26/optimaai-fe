          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {templates.map((tpl) => (
              <TemplateCard
                ...
                isLoading={generate.isPending && selectedTemplate?.id === tpl.id}
      ...
      {generate.isPending && (
        <div className="glass-card rounded-2xl border border-border p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            This usually takes 20–40 seconds. The AI is analysing your ML results and uploaded data,
            section by section.
          </p>
        </div>
      )}
    ...
    >
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-1">{template.section_count} sections</p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {report.sections.map((sec) => (
          <section key={sec.id} className="space-y-3">
            <h3 className="text-lg font-semibold border-b border-border pb-2">{sec.title}</h3>
            <MarkdownContent variant="full">{sec.content_md}</MarkdownContent>
          </section>
        ))}
      </div>
    </article>
