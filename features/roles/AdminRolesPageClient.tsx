          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="text-left px-4 py-3 font-semibold text-muted-foreground min-w-[200px]">
                  Permission
                </th>
                {roles.map((role) => (
                  <th
                    key={role.id}
                    className="text-center px-4 py-3 font-semibold text-muted-foreground"
                  >
                    <div className="flex flex-col items-center gap-1">
                      <Shield className="w-4 h-4" />
                      <span>{ROLE_LABELS[role.name]}</span>
                    </div>
                  </th>
                ))}
              </tr>
                ...
                <Fragment key={`group-${group.label}`}>
                  ...
                  {group.permissions.map((permission) => (
                    ...
                    >
                      ...
                      {roles.map((role) => {
                        const has = role.permissions.some((item) => item.key === permission);
                        return (
                          <td key={`${role.id}-${permission}`} className="text-center px-4 py-2.5">
                            {has ? (
