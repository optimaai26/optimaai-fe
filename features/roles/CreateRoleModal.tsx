  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-fade-in">
      <div className="bg-background/95 backdrop-blur-xl border border-border/50 rounded-2xl shadow-2xl ring-1 ring-white/10 w-full max-w-2xl max-h-[85vh] flex flex-col animate-slide-up">
        ...
        >
          <div>
            <label htmlFor="role-name" className="block text-sm font-medium text-foreground mb-1">
              Role Name
            </label>
            <input
              id="role-name"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {PERMISSION_GROUPS.map((group) => {
                ...
                return (
                  <div
                    key={group.label}
                    className="border border-border/50 rounded-xl p-4 bg-muted/10 hover:bg-muted/20 hover:border-border/80 transition-colors"
                  >
                    ...
                    <div className="space-y-2.5">
                      {group.permissions.map((perm) => (
                        <label key={perm} className="flex items-center gap-3 cursor-pointer group">
                          <div
                            className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${selectedPermissions.has(perm) ? 'bg-primary border-primary text-primary-foreground shadow-sm shadow-primary/20' : 'border-input bg-background group-hover:border-primary/50'}`}
                          >
