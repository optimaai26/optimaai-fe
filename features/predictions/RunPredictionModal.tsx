...
import { AlertTriangle, CheckCircle2, ChevronLeft, Loader2, Play, Sparkles, X } from 'lucide-react';
...
export function RunPredictionModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const availableColumns = useMemo(() => {
    ...
    const dateSources = Object.values(mapping)
      .filter((v): v is string => v?.startsWith('_from_date:') ?? false)
          ...
          {step === 'choose' && (
            <>
              <div>
                ...
                <select
                  value={uploadId ?? ''}
                  onChange={(e) => setUploadId(e.target.value ? Number(e.target.value) : null)}
                ...
                >
                  <option className="dark:bg-surface-800 dark:text-foreground" value="">
                    — Choose a dataset —
                  </option>
                  {datasets.map((d) => (
                    <option
                      className="dark:bg-surface-800 dark:text-foreground"
                      key={d.id}
                      value={d.id}
                    >
              ...
              {uploadId !== null && (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-sm font-medium">Recommended predictions</span>
                  ...
                  {rankedSuggestions && (
                    <div className="space-y-2">
                      {rankedSuggestions.map(({ suggestion, score }, idx) => {
                        ...
                        const totalFeatures = Object.keys(suggestion.mapping).length;
                        const mappedCount = totalFeatures - suggestion.missing.length;
                        const toneColor =
                          tone === 'good'
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => handlePickModel(suggestion.model_kind)}
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs mt-2">
                                  ...
                                  <span className="text-muted-foreground">features mapped</span>
          ...
          {step === 'map' && chosenSuggestion && modelKind && (
            <>
              <div className="text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground">every row in your dataset</span> using
                this mapping.
              </div>
                ...
                <div className="flex items-start gap-2 p-3 rounded-lg bg-warning/10 border border-warning/30">
                  ...
                  <div className="text-xs">
                    <div className="font-medium text-warning mb-1">
                      {chosenSuggestion.missing.length} feature(s) couldn't be auto-mapped
                    </div>
                    <div className="text-muted-foreground">
                      Missing: {chosenSuggestion.missing.join(', ')}. The prediction will run with
                      defaults for these.
                    </div>
                  </div>
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        {isDerived && (
                          <div className="text-xs text-muted-foreground">derived from date</div>
                        )}
                      </div>
...
function BatchResultView({ result, kind }: { result: AnyBatchResult; kind: ModelKind }) {
  return (
    <div className="space-y-4">
      ...
      {kind === 'revenue' && <RevenueResultView summary={result.summary as BatchRevenueSummary} />}
      {kind === 'churn' && <ChurnResultView summary={result.summary as BatchChurnSummary} />}
      {kind === 'growth' && <GrowthResultView summary={result.summary as BatchGrowthSummary} />}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        ...
        <Stat label="Total predicted" value={`$${summary.total_predicted.toLocaleString()}`} />
        <Stat label="Avg per row" value={`$${summary.avg_row_predicted.toLocaleString()}`} />
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-3">
        ...
        <Stat label="High risk" value={summary.risk_counts.high.toLocaleString()} tone="danger" />
        <Stat
          label="Medium risk"
          value={summary.risk_counts.medium.toLocaleString()}
          tone="warning"
        />
        <Stat label="Low risk" value={summary.risk_counts.low.toLocaleString()} tone="success" />
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <Stat label="Months of history" value={summary.n_months_actual.toLocaleString()} />
        <Stat label="Forecast horizon" value={`${summary.n_months_forecast} months`} />
      </div>
