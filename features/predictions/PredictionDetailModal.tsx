		...
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-fade-in">
			<div className="bg-background/95 backdrop-blur-xl border border-border/50 rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col animate-slide-up max-h-[90vh]">
				...
				<div className="flex-1 overflow-y-auto p-6 space-y-4">
					...
					{isError && (
						<div className="text-sm text-danger">Failed to load prediction.</div>
					)}
...
function DetailBody({ prediction }: { prediction: PastPrediction }) {
	const summary = prediction.result?.data as
		...
		<div className="space-y-4">
			<div className="grid grid-cols-3 gap-3">
				<Stat
					...
					value={(summary.n_rows ?? 0).toLocaleString()}
				/>
				<Stat
				...
				<Stat
					label="Avg per row"
					value={`$${(summary.avg_row_predicted ?? 0).toLocaleString()}`}
				/>
			</div>
			{chartData.length > 0 && (
	...
	return (
		<div className="space-y-4">
			<div className="grid grid-cols-4 gap-3">
				<Stat
					label="Customers"
					value={(summary.n_customers ?? 0).toLocaleString()}
				/>
				<Stat label="High risk" value={rc.high.toLocaleString()} tone="danger" />
				<Stat
					label="Medium risk"
					value={rc.medium.toLocaleString()}
					tone="warning"
				/>
				<Stat
					label="Low risk"
					value={rc.low.toLocaleString()}
					tone="success"
				/>
			</div>
		...
		<div className="space-y-4">
			<div className="grid grid-cols-2 gap-3">
				<Stat
					...
					value={(summary.n_months_actual ?? 0).toLocaleString()}
				/>
				<Stat
					label="Forecast horizon"
					value={`${summary.n_months_forecast ?? 0} months`}
				/>
			</div>
			{chartData.length > 0 && (
				<div className="glass-card p-4 rounded-xl">
