							...
							<div className="flex items-start justify-between mb-4">
								<div>
									<div className="flex items-center gap-2 mb-2">
										<div
											className={`px-2 py-0.5 rounded text-xs font-semibold capitalize border ${getStatusColor(prediction.status)} flex items-center gap-1.5 w-fit`}
										>
									...
									<h3 className="font-semibold text-lg text-foreground group-hover:text-primary transition-colors capitalize">
										{prediction.type.replace("_", " ")} Model
									</h3>
								</div>
							...
							<div className="flex-1">
								<p className="text-sm text-muted-foreground pr-4 line-clamp-3 leading-relaxed">
									{prediction.result?.summary ??
										"This model is currently processing data. Results will be available shortly after completion."}
								</p>
							</div>
					...
					{predictions.length === 0 && (
						<div className="col-span-full min-h-[160px] flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl text-muted-foreground">
							...
							<p className="text-xs mt-1">
								Start by running the new prediction wizard.
							</p>
						</div>
					)}
				</div>
