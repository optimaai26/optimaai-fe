...
export function BmcPageClient() {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      >
        <label htmlFor="business-name" className="text-sm font-medium shrink-0">
          Business name
        </label>
        <input
          id="business-name"
        ...
        {bmc?.bmc_id && bmc.files && (
          <div className="flex gap-2 ml-auto">
            <DownloadButton url={getBmcDownloadUrl(bmc.bmc_id, 'pdf')} label="PDF" />
            <DownloadButton url={getBmcDownloadUrl(bmc.bmc_id, 'docx')} label="Word" />
          </div>
        )}
      </form>
      ...
      {generate.isPending && !bmc && (
        <div className="glass-card rounded-2xl border border-border p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Analysing each block from your uploaded data — this usually takes 60–90 seconds.
          </p>
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card rounded-xl border border-border p-3 text-xs text-muted-foreground flex items-center gap-3 flex-wrap">
        ...
        <span className="ml-auto">Generated {new Date(bmc.generated_at).toLocaleString()}</span>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 auto-rows-[minmax(220px,_auto)]">
        ...
        <Block block={byId.cost_structure} className="col-span-2" accent="cost" />
        {/* Row 3 cols 3-5 — Revenue Streams */}
        <Block block={byId.revenue_streams} className="col-span-3" accent="revenue" />
      </div>

      {/* Confidence legend */}
      <div className="glass-card rounded-xl border border-border p-3 text-xs flex items-center gap-4 flex-wrap">
        <span className="font-medium text-muted-foreground">Block confidence:</span>
      ...
      {/* Evidence-tag legend */}
      <div className="glass-card rounded-xl border border-border p-3 text-xs flex items-center gap-4 flex-wrap">
        <span className="font-medium text-muted-foreground">Statement evidence:</span>
...
function ConfidenceLegend({ level, label }: { level: BmcBlock['confidence']; label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5">
      <span
