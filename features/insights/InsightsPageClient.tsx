	...
	return (
		<div className="animate-fade-in space-y-6">
			...
			{isLoading ? (
				...
				<div className="text-sm text-danger">Failed to load insights.</div>
			) : (
				<>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						{insights.map((insight) => (
							<div
								key={insight.id}
								className="glass-card rounded-2xl p-6 border border-border min-h-[220px]"
							>
								...
								<div className="pt-4 border-t border-border mt-auto text-xs text-muted-foreground flex justify-between">
					...
					<div className="glass-card p-8 rounded-2xl border border-dashed border-border bg-primary/5 flex items-center gap-6">
						...
						<div>
							...
							<p className="text-sm text-muted-foreground mt-1">
								These cards are now driven by query data instead of hardcoded
								placeholders.
							</p>
						</div>
					</div>
