...
import {
    ...
    useKbStats,
    type RagAnswer,
} from '@/features/knowledge-base/useKnowledgeBase';
...
export function InsightsPageClient() {
    ...
    const submit = async (text: string) => {
        ...
        setTurns((prev) => [
            ...prev,
            { id: turnId, question: q, timestamp: Date.now() },
        ]);
        setQuestion('');
    ...
    return (
        <div className="text-center py-10">
            ...
            <p className="text-sm text-muted-foreground max-w-md mx-auto mt-2">
                Your question will be answered using only the data you&apos;ve
                uploaded — no hallucinations, every claim cites a source.
            </p>
                ...
                {answer.sources && answer.sources.length > 0 && (
                    <div className="pt-3 mt-2 border-t border-border flex flex-wrap gap-2 text-xs text-muted-foreground">
                        ...
                        {Array.from(new Set(answer.sources)).map((s) => (
                            <span
                                key={s}
                                className="px-2 py-0.5 rounded-md bg-background border border-border"
                            >
                                {s}
                            </span>
                        ))}
