...
const columns: Column<Dataset>[] = [
  {
    ...
    header: 'Name',
    render: (row) => (
      <Link href={`/datasets/${row.id}`} className="font-medium hover:text-primary">
        {row.name}
      </Link>
    ),
  },
  ...
  { key: 'columnCount', header: 'Columns' },
  {
    key: 'actions',
    header: 'Actions',
    render: (row) => <DeleteDatasetButton id={row.id} />,
  },
];
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="text-sm text-danger">Failed to load datasets.</div>
      ) : datasets.length === 0 ? (
        ...
        <DataTable columns={columns} data={datasets} keyExtractor={(row) => row.id} />
      )}
