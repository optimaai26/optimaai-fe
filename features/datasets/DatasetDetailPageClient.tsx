...
import { type DatasetView, useDatasetPreview } from '@/features/datasets/useDatasetPreview';
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          <PageHeader title={dataset.name} description={`Source file: ${dataset.fileName}`} />

          {/* Summary tiles */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatTile label="Rows" value={dataset.rowCount.toLocaleString()} />
            <StatTile label="Columns" value={dataset.columnCount.toLocaleString()} />
            <StatTile label="Status" value={String(dataset.status ?? '—')} capitalize />
          </div>
...
function StatTile({
  ...
  return (
    <div className="glass-card h-28 rounded-xl flex flex-col items-center justify-center">
      ...
      <span className={`text-2xl font-bold mt-1 ${capitalize ? 'capitalize' : ''}`}>{value}</span>
    </div>
  );
}
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {data.rows.map((row, idx) => (
              ...
              >
                {data.columns.map((col) => (
                  <td key={col} className="px-3 py-2 whitespace-nowrap font-mono text-[11px]">
                    {formatCell(row[col])}
                  </td>
                ))}
              </tr>
